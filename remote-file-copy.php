...
<ul class="progress-notice"></ul>
<progress value="0" max="100" style="display:none;"></progress>
            <div id="upload-progress" class="progress" style="display:none; height: 25px;">
                <div class="progress-bar progress-bar-striped progress-bar-animated bg-info" style="height: 25px;"></div>
            </div>
    <!-- Optional JavaScript -->
...
function callback(message) {
    ...
    $.each(message, function (key, value) {
        switch (key) {
            case 'send':
                $('#upload-progress').show(( "fade", { direction: "in" } ));
                break;
            case 'progress':
                if (value && value.total) {
                    //$('progress').val(value.loaded / value.total * 100);
                    var percent = value.loaded / value.total * 100;
                    $("#upload-progress .progress-bar").css("width", +percent + "%").attr("aria-valuenow", percent).text(percent + "% Complete").fadeIn("slow");
                }
                break;
            case 'done':
                $('<li class="alert alert-success">').text(value && value.name + ' Download complete!').appendTo('ul').fadeIn("slow");
                $('#upload-progress').hide( "fade", { direction: "out" }, 3000 );
                break;
            case 'fail':
                $('<li class="alert alert-danger">').text(value && value.message).appendTo('ul').fadeIn("slow");
                $('#upload-progress').hide( "fade", { direction: "out" }, 3000 );
                break;
        }
    });
